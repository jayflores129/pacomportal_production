...
<div class="panel panel-top">
  {!! Breadcrumbs::render('addFirmwares') !!}
</div>
<div class="row">
      ...
      <div class="panel panel-default upload-tab">
        <div class="panel-body">
           @include('components/errors')
             ...
             <div class="form">
                <h2>Upload Software / Firmware</h2>
                {!! Form::open([
                    ...
                    <div class="form-group">
                       <input type="checkbox" name="latest" id="latestFirmware"/> <strong class="latest-label"> Latest software/firmware file</strong>
                    </div>
                    <div class="form-group">
                        <label for="InputFile">File Name</label>
                        <input type="text" name="name" class="form-control" id="firmwareName" required/>
                    </div>  
                    <div class="form-group">
                        <label for="selectCat">Category Name</label>
                        @if($categories)
                    ...
                    <div class="form-group">
                        <label for="InputFile">Add a link</label>
                        <div class="firmware-wrap">
                           <input type="text" name="firmware_link"  class="form-control" id="firmware_link"/>
                        </div>
                    </div>
...
<script>
  ...
  $('#btnLoad').on('click', function(e){
      ...
      firmwareLink = $('#firmware_link').val(),
      category     = $('#firmware'),
      ...
      if( fileName.val() == '') {
          fileName.addClass('input-error');
          error_count++;
      }  else if( fileName.val() == 'new' ) {
          $('#fileName').addClass('input-error');
          error_count++;
      } else {
          $('#fileName').removeClass('input-error');
          if( fileName.hasClass('input-error') ) {
             fileName.removeClass('input-error');
             error_count--;
          } 
      }
      ...
      var fimrware_parentEl  = $('#firmware_link').closest('.firmware-wrap');

      // Validate File Upload

      if( inputFile.value == '' && firmwareLink == ''  ) {
        
          fimrware_parentEl.addClass('input-error');
          ...
          if(fimrware_parentEl.hasClass('input-error') ) {
             fimrware_parentEl.removeClass('input-error');
          } else {
            fimrware_parentEl.removeClass('input-error');
          }
      ...
      var doc_parentElement = $('#documentInput').closest('.file-upload-wrapper');

      if( doc.value == '' ) {
          doc_parentElement.addClass('input-error');
          error_count++;

      } else {
          if( doc_parentElement.hasClass('input-error') ) {
             doc_parentElement.removeClass('input-error');
             error_count--;
          } else {
            doc_parentElement.removeClass('input-error');
          }
      ...
      display_table(table,  file, docu, rel, cat, version.val(), fileName.val(), firmwareLink );
 ...
 $('#table-listing').on( 'click', '.submit-file', async function(e) {
      ...
      if(document.getElementById("latestFirmware").checked == true) {
          latest = 1;
      } else {
          latest = 0;
      }
      ...
      var file_data = inputFile.files[0]
          ...
          form_data.append('firmware_link', firmwareLink );
          ...
          form_data.append('is_latest', latest);
             ...
             xhr: function () {
              ...
              success: function(result)
              {
                 ...
                 if(result == 'yes') {
                     ...
                     setInterval(function(){ location.href = '{{ url('firmwares') }}' }, 1500);
   ...
   */
 function display_table(table, file, docu, release_select, name, version, fileName, firmwareLink ) {
        ...
        $output = '';
            ...
            $output += '<tr>';
              ...
              if(file) {
                $output += '<td width="30%"><div><input type="hidden" class="file-info" value="'+ file +'" /><span>Firmware Name </span> : '+ fileName +'</div><span>Filesize</span> : '+ bytesToSize(file.size) +'</span>';
              } else {
                $output += '<td width="30%"><div><input type="hidden" class="file-info" value="'+ firmwareLink +'" /><span>Firmware Name</span> : '+ fileName +'</div><span class="label label-danger">Make sure the filename and file extension is correct</span>';
              }
              $output += '<div><span>Filename</span> : '+ docu.name +'</div><span>Filesize</span> : '+  bytesToSize(docu.size) +'</span></td>';
              $output += '<td>';
                ...
                $output += ' <button class="hide btn btn-danger remove-file">Click to upload more</button>';
              $output += '</td>';  
            $output += '</tr>';
