                 ...
                 <table class="table table-default-brand table-striped ">
                    <thead>
                     <tr>
                         ...
                         <th style="background: #f5f5f5;border-left: 1px solid #ddd;border-right: 1px solid #ddd;">Total Faulty Items</th>
                         <th style="background: #f5f5f5;border-left: 1px solid #ddd;border-right: 1px solid #ddd;">
                      ...
                      case 'Confirmed':
                        ...
                        case 'Submitted':
                         ...
                         <tr>
                             ...
                             <td width="150">
                                <a href="/repairs/{{$repair->id}}" class="btn btn-sm btn-primary" target="_blank"><span class="fa fa-eye"></span> View Details</a>
                             </td> 
                         </tr>
  ...
  <script>
    ...
    Array.from(document.querySelectorAll('.sort-button')).forEach(elem => {
        elem.addEventListener('click', (e) => {
            ...
            const sortDirection = search.get('sort_direction') || 'asc';
            ...
            location.href = "/advanced-search-rma?" + search.toString();
        })
    })
  </script>

  <?php $isSearch = isset($_GET['is_search']) ? $_GET['is_search'] : 0; ?>
...
@section('js')
    ...
    <script>
       ...
       var status = '',
           ...
           value = '',
           company = '',
           rma_ids = <?php echo json_encode($rma_ids) ?>,
           is_search = <?php echo $isSearch ?>;


      //Set local Storage for RMA

      //$(document).on('ready',function(){

        if (is_search == 1) {
          //console.log(<?php echo $rma_ids ?>);
          localStorage.setItem("RmaIDs", JSON.stringify(rma_ids));
          //$('#print_rma_ids_input_csv').val(<?php echo $rma_ids ?>);
        } else {
          localStorage.removeItem("RmaIDs");
        }
        
        var RmaIDs = localStorage.getItem("RmaIDs");
      ...
      function getURLparams() {
        ...
        var url        = '<?php echo url('search-rma') ?>';

        var new_adv_search = url + '?' + urlParams;
        $('#advanced-search-buttonx').attr('href', new_adv_search);
      }
      ...
      $('#searchSubmit').on('click', function(e) {
        ...
        $('.pagination-links').on('click', '.page-link', function(e){
            ...
            var link    = '{{ ( Auth::user()->isAdmin() === false ) ? URL::to('searchUserRepair') : URL::to('searchRepair') }}';
            var output2 = '';
        ...
        function listView( ticket = [] ) {
             ...
             var requested_date     = ticket['requested_date'];
                ...
                output =  '<tr><td>R' + id + '</td>'+
                              ...
                              '<td><a href="'+ '/repairs' +'/'+ id +'" target="_blank" class="btn btn-sm btn-primary"><span class="fa fa-eye"></span> View Details</a></td></tr>';
